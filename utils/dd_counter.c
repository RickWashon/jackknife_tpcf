...
static inline int64_t coord_to_cell(const double x, const double inv_cellsize, const int64_t nside) {
    ...
    if (ix < 0) ix = 0;
    else if (ix >= nside) ix = nside - 1;
    return ix;
}

static inline double axis_min_sep_periodic(const int64_t i1, const int64_t i2, const int64_t nside, const double cellsize) {
    int64_t d = i1 - i2;
    if (d < 0) d = -d;
    if (d > (nside - d)) d = nside - d;
    if (d <= 1) return 0.0;
...
static inline __attribute__((always_inline)) int64_t locate_bin_lut(
    ...
    if (r2 < r2_min || r2 >= r2_max) return -1;
    ...
    if (r2 < rbins2[b]) b--;
    else if (r2 >= rbins2[b + 1]) b++;
    while (b > 0 && r2 < rbins2[b]) b--;
    while (b < nbins - 1 && r2 >= rbins2[b + 1]) b++;

    if (r2 >= rbins2[b] && r2 < rbins2[b + 1]) return b;
    return -1;
}
...
int dd_auto_no_weight(
    ...
    if (x == NULL || y == NULL || z == NULL || rbins == NULL || dd_total == NULL) return -1;
    if (npts < 0 || nbins <= 0 || boxsize <= 0.0) return -2;

    for (i = 0; i < nbins; i++) dd_total[i] = 0ULL;
    if (npts < 2) return 0;

    double *rbins2 = (double *)malloc((size_t)(nbins + 1) * sizeof(double));
    if (rbins2 == NULL) return -3;
    for (i = 0; i < nbins + 1; i++) rbins2[i] = rbins[i] * rbins[i];
    ...
    }
    {
        ...
        for (t = 0; t < lut_n; t++) {
            ...
            while (cur < nbins - 1 && probe >= rbins2[cur + 1]) cur++;
    ...
    if (requested_cell <= 0.0) requested_cell = boxsize / 10.0;
    if (requested_cell <= 0.0) requested_cell = rmax;
    ...
    nside *= (int64_t)rf;
    if (nside < 1) nside = 1;
    if (nside > (int64_t)mcpd) nside = (int64_t)mcpd;

    {
        ...
        if (nside > max_side) nside = max_side;
    }
    ...
    if (cell_count == NULL || cell_offset == NULL || cell_cursor == NULL || point_cell == NULL || cell_points == NULL || nonempty_flag == NULL) {
        ...
        free(cell_count); free(cell_offset); free(cell_cursor);
        free(point_cell); free(cell_points); free(nonempty_flag);
        return -4;
    }
    ...
    for (i = 0; i < ncells; i++) {
        ...
        if (cell_count[i] > 0) nonempty_flag[i] = 1;
    }
    ...
    for (i = 0; i < ncells; i++) if (cell_count[i] > 0) n_nonempty += 1;
    ...
    if (nonempty_cells == NULL || nonempty_ix == NULL || nonempty_iy == NULL || nonempty_iz == NULL) {
        ...
        free(cell_count); free(cell_offset); free(cell_cursor);
        free(point_cell); free(cell_points); free(nonempty_flag);
        free(nonempty_cells); free(nonempty_ix); free(nonempty_iy); free(nonempty_iz);
        return -6;
    }
    ...
    if (nbr_counts == NULL || nbr_offsets == NULL) {
        ...
        free(point_cell); free(cell_points); free(nonempty_flag);
        free(nonempty_cells); free(nonempty_ix); free(nonempty_iy); free(nonempty_iz);
        free(nbr_counts); free(nbr_offsets);
        return -8;
    }
...
#pragma omp parallel for schedule(dynamic, 32) num_threads(nth) reduction(|:bad_nbr_count)
    ...
    for (int64_t idx1 = 0; idx1 < n_nonempty; idx1++) {
        ...
        for (int64_t dx_cell = -reach; dx_cell <= reach; dx_cell++) {
            for (int64_t dy_cell = -reach; dy_cell <= reach; dy_cell++) {
                for (int64_t dz_cell = -reach; dz_cell <= reach; dz_cell++) {
                    ...
                    if (!nonempty_flag[c2] || c2 < c1) continue;
                    {
                        ...
                        if (min_r2 >= r2_max || max_r2 < r2_min) continue;
                    }
                    cnt++;
                }
            }
        ...
        nbr_counts[idx1] = cnt;
        if (cnt > max_nbr) bad_nbr_count = 1;
    }
    if (bad_nbr_count) {
        free(rbins2); free(bin_lut);
        free(cell_count); free(cell_offset); free(cell_cursor);
    ...
    nbr_offsets[0] = 0;
    for (int64_t idx1 = 0; idx1 < n_nonempty; idx1++) nbr_offsets[idx1 + 1] = nbr_offsets[idx1] + nbr_counts[idx1];
    ...
    if (nbr_cells == NULL || nbr_shiftx == NULL || nbr_shifty == NULL || nbr_shiftz == NULL) {
        ...
        free(nonempty_cells); free(nonempty_ix); free(nonempty_iy); free(nonempty_iz);
        free(nbr_counts); free(nbr_offsets);
        free(nbr_cells); free(nbr_shiftx); free(nbr_shifty); free(nbr_shiftz);
        return -10;
    }
...
#pragma omp parallel for schedule(dynamic, 32) num_threads(nth) reduction(|:bad_nbr_fill)
    ...
    if (x_sorted == NULL || y_sorted == NULL || z_sorted == NULL) {
        free(rbins2); free(bin_lut);
        free(cell_count); free(cell_offset); free(cell_cursor);
        free(point_cell); free(cell_points); free(nonempty_flag);
        free(nonempty_cells); free(nonempty_ix); free(nonempty_iy); free(nonempty_iz);
        free(nbr_counts); free(nbr_offsets);
        free(nbr_cells); free(nbr_shiftx); free(nbr_shifty); free(nbr_shiftz);
        free(x_sorted); free(y_sorted); free(z_sorted);
        return -11;
    }
...
#pragma omp for schedule(dynamic, 8)
#endif
        for (int64_t idx1 = 0; idx1 < n_nonempty; idx1++) {
            const int64_t kk_begin = nbr_offsets[idx1];
            const int64_t kk_end = nbr_offsets[idx1 + 1];
            const int64_t c1 = nonempty_cells[idx1];
                ...
                if (c2 == c1) {
                    for (int64_t a = s1; a < e1 - 1; a++) {
...
#if defined(__AVX2__)
                        {
                            ...
                            for (; bidx + 3 < e1; bidx += 4) {
                                ...
                                for (int lane = 0; lane < 4; lane++) {
                                    const int64_t b = locate_bin_lut(r2buf[lane], rbins2, nbins, r2_min, r2_max, bin_lut, lut_n, inv_lut_step);
                                    if (b >= 0) local_total[b] += 2ULL;
                                }
                            }
                        }
#endif
                        for (; bidx < e1; bidx++) {
                            ...
                            const int64_t b = locate_bin_lut(r2_eval(dx, dy, dz, use_float32), rbins2, nbins, r2_min, r2_max, bin_lut, lut_n, inv_lut_step);
                            if (b >= 0) local_total[b] += 2ULL;
                        }
                    }
                } else {
...
#endif
                        for (; bidx < e2; bidx++) {
                            const double dx = (x_sorted[bidx] + sx) - xi;
                            const double dy = (y_sorted[bidx] + sy) - yi;
                            const double dz = (z_sorted[bidx] + sz) - zi;
                            const int64_t b = locate_bin_lut(r2_eval(dx, dy, dz, use_float32), rbins2, nbins, r2_min, r2_max, bin_lut, lut_n, inv_lut_step);
                            if (b >= 0) local_total[b] += 2ULL;
                        }
                    }
    ...
    for (int t = 0; t < nth; t++) {
        ...
        for (i = 0; i < nbins; i++) dd_total[i] += lt[i];
    }
